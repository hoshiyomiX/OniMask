...
// -----------------------------------------------------------------
// Note: Detection hiding (dl_iterate_phdr, openat, readlink filtering)
// has been removed following the ReZygisk approach. The framework no
// longer hooks any libart.so PLT symbols that modules commonly use,
// eliminating symbol conflicts with zygisk modules. The tradeoff is
// zero detection hiding. If detection hiding is needed in the future,
// chain-aware hooks must be implemented to avoid clobbering module
// hooks installed during preAppSpecialize().
